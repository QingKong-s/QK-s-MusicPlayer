...
{
    IWICBitmap* pwicb;
    g_pWICFactory->CreateBitmap(m_cxLeftBK, m_cyLeftBK, GUID_WICPixelFormat32bppPBGRA, WICBitmapCacheOnDemand, &pwicb);
    ...
    ID2D1Bitmap* pd2db;
    HRESULT hr = m_pD2DDCLeftBK->CreateBitmapFromWicBitmap(pwicb, &pd2db);
    ...
    ID2D1GdiInteropRenderTarget* p;
    m_pD2DDCLeftBK->QueryInterface(__uuidof(ID2D1GdiInteropRenderTarget), (void**)&p);
    HDC hDC;
    hr = p->GetDC(D2D1_DC_INITIALIZE_MODE_COPY, &hDC);
    ...
    ////g_pD2DFactory->CreateWicBitmapRenderTarget()
    //ID2D1Bitmap* pp;
    ...
    ///*
    //ID2D1Bitmap* ppp;
    //m_pD2DBmpLeftBK2->QueryInterface(__uuidof(ID2D1Bitmap), (void**)&ppp);
    //hr = m_pD2DRTLeftBK->CreateSharedBitmap(__uuidof(ID2D1Bitmap), ppp, &D2DBmpProp, &pp);

    //ID2D1BitmapBrush* pbr;
    //
    //hr = m_pD2DRTLeftBK->CreateBitmapBrush(m_pD2DBmpLeftBK2, &pbr);*/
    //D2D1_SIZE_U sizeu = { m_cxLeftBK,m_cyLeftBK };
    ////hr = m_pD2DRTLeftBK->CreateBitmapFromWicBitmap(pwicb, &pp);
    //m_pD2DRTLeftBK->CreateBitmap(sizeu, D2DBmpProp, &pp);
    //D2D1_POINT_2U ptu = { 0,0 };
    //D2D1_RECT_U rcu = { 0,0,m_cxLeftBK,m_cyLeftBK };
    //hr = pp->CopyFromBitmap(&ptu, m_pD2DBmpLeftBK2, &rcu);
    //m_pD2DRTLeftBK->BeginDraw();
    //m_pD2DRTLeftBK->DrawBitmap(pp);
    //hr = m_pD2DRTLeftBK->EndDraw();
...
void UI_VEDrawWaves()
{
    ...
    if (m_uThreadFlagWaves == THREADFLAG_STOPED)
    {
        ...
        m_pD2DDCLeftBK->SetAntialiasMode(D2D1_ANTIALIAS_MODE_FORCE_DWORD);
        // 上面是右声道，下面是左声道
        while (true)// 向右画
        {
            ...
            m_pD2DDCLeftBK->DrawLine(D2DPtF1, D2DPtF2, m_pD2DBrMyBlue, DPIS_CXWAVESLINE);
        ...
        while (true)// 向左画
        {
            ...
            m_pD2DDCLeftBK->DrawLine(D2DPtF1, D2DPtF2, m_pD2DBrMyBlue, DPIS_CXWAVESLINE);
            //MoveToEx(m_hcdcLeftBK2, x, y, NULL);
            //LineTo(m_hcdcLeftBK2, x, y - (int)(HIWORD(m_dwWavesData[i]) / 32768.0f * (float)DPIS_CYSPEHALF));
            //MoveToEx(m_hcdcLeftBK2, x, y, NULL);
        ...
        D2DPtF1 = { (float)x,(float)m_yWaves };
        D2DPtF2 = { (float)x,(float)(m_yWaves + DPIS_CYSPE) };
    ...
    m_pD2DDCLeftBK->QueryInterface(__uuidof(ID2D1GdiInteropRenderTarget), (void**)&p);
    HDC hDC;
    HRESULT hr = p->GetDC(D2D1_DC_INITIALIZE_MODE_COPY, &hDC);
...
LRESULT CALLBACK WndProc_TBGhost(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    //    IWICBitmap* pWICBmpResult;

    //    g_pWICFactory->CreateBitmap(cxMax, cyMax, GUID_WICPixelFormat32bppPBGRA, WICBitmapNoCache, &pWICBmpResult);
    //    WICRect WICrc=;

    //    pWICBmpScaled->CopyPixels(,)
    //    
...
LRESULT CALLBACK WndProc_LeftBK(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
    {
        ...
        DeleteDC(m_hcdcLeftBK);
        DeleteDC(m_hcdcLeftBK2);
        DeleteObject(m_hbmpLeftBK);
        DeleteObject(m_hbmpLeftBK2);
        HDC hDC = GetDC(hWnd);
        m_hcdcLeftBK = CreateCompatibleDC(hDC);
        m_hcdcLeftBK2 = CreateCompatibleDC(hDC);

        m_hbmpLeftBK = CreateCompatibleBitmap(hDC, m_cxLeftBK, m_cyLeftBK);
        m_hbmpLeftBK2 = CreateCompatibleBitmap(hDC, m_cxLeftBK, m_cyLeftBK);
  ...
  //      hr = m_pD2DDCLeftBK->CreateBitmapFromDxgiSurface(m_pDXGISfceLeftBK, &D2DBmpProp, &m_pD2DBmpLeftBK);
        ...
        m_pD2DBmpLeftBK->Release();
        m_pDXGISfceLeftBK->Release();
        HRESULT hr = m_pDXGIScLeftBK->ResizeBuffers(0, m_cxLeftBK, m_cyLeftBK, DXGI_FORMAT_UNKNOWN, 0);
